...
class ColorSquareDetector:
    ...
    def update_color_range(
        ...
        self._color_ranges[color_name]["lower"] = np.array(
            hsv_min,
            dtype=np.uint8,
        )
        self._color_ranges[color_name]["upper"] = np.array(
            hsv_max,
            dtype=np.uint8,
        )
    ...
    def _create_color_mask(self, img: cv2t.MatLike, color_name: str) -> cv2t.MatLike:
        ...
        lower = self._color_ranges[color_name]["lower"]
        upper = self._color_ranges[color_name]["upper"]
...
def main() -> NoReturn:
    ...
    # 色の範囲パラメータを格納する辞書
    color_params: dict[str, dict[str, DoubleSubscriber]] = {}
    ...
    for color_name, color_info in colors.items():
        ...
        color_table = color_ranges_table.getSubTable(color_name)

        for param_name, default_value in color_info.items():
            color_params[color_name][param_name] = get_double_subscriber(
                color_table,
                param_name,
                default_value,
            )
    ...
    while True:
        ...
        for color_name, params in color_params.items():
            ...
            detector.update_color_range(color_name_lower, hsv_min, hsv_max)
